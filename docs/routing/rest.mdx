---
sidebar_position: 1
ai_summary: "REST verbs (GET, POST, PUT, PATCH, DELETE) can be used to define routes. Top-level routes require explicit controller and action. Supports Koa middleware as second argument or in array for multiple middleware."
---
...
You can provide Koa middleware directly to a route by sending a callback function as the second argument to any method, i.e.:

```ts
r.get('ping', async (ctx, next) => {
