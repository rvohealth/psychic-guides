---
sidebar_position: 2
ai_summary: "Resourceful routing provides CRUD routes (index, create, show, update, destroy) for resources. Supports nested resources, only/except options to limit routes, singular resources, and custom member/collection routes. Routes automatically map to controller actions."
---
...
A route declared directly inside the `resources`/`resource` callback — outside a `collection` block — is **member-scoped**: Psychic automatically prepends `:id` to it, as with `edit` above. There is no `r.member` method; use the existing verb methods (`r.get`/`r.post`/etc.) for a member route, and read the id inside the action with `this.castParam('id', 'uuid')` — cast it to whatever type your app's primary key actually is (`uuid`, `bigint`, or `integer`), never `'string'`.
