...
ai_summary: >-
  Encrypt class provides symmetric encryption (AES-GCM algorithms: aes-256-gcm,
  aes-192-gcm, aes-128-gcm). Keys must be base64-encoded with specific bit
  lengths. Use Encrypt.generateKey() to generate keys. Never hard-code keys -
  use environment variables or AWS SecretsManager.
---
...
A light-weight `Encrypt` class is provided by `Dream`, which can be freely utilized throughout your app for symmetric encryption tasks.
...
Encryption keys are an essential part of any encryption algorithm, and _must_ be provided in specific formats to be considered valid for a given encryption algorithm. For example, for `aes-256-gcm`, a 256-bit (32-byte) encryption key must be provided in base64 encoding. The list of requirements is as follows:

- `aes-256-gcm`: 256-bit (32-byte), base64-encoded string
- `aes-192-gcm`: 192-bit (24-byte), base64-encoded string
- `aes-128-gcm`: 128-bit (16-byte), base64-encoded string
...
<PackageManagerTabs
  code={`NODE_ENV=development {{PM}} console`}
/>
