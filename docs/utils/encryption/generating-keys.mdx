...
sidebar_position: 1
ai_summary: >-
  Encrypt.generateKey() generates encryption keys for specified algorithms. Keys
  are algorithm-specific and must match encryption/decryption algorithm. Never
  hard-code keys - use environment variables or AWS SecretsManager. Use
  different keys in local/test vs production.
