...
ClockTimeTz throws `InvalidClockTimeTz` errors for invalid operations. There is no "invalid instance" to inspect — every parser throws immediately on unparsable input, so catch the specific error rather than probing the result. `InvalidClockTimeTz` is exported from `@rvoh/dream/errors`, not `@rvoh/dream`:

```ts
import { ClockTimeTz } from '@rvoh/dream'
import { InvalidClockTimeTz } from '@rvoh/dream/errors'
