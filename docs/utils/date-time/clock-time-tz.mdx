---
sidebar_position: 5
ai_summary: 'ClockTimeTz represents time with timezone (TIME WITH TIME ZONE in Postgres). Supports microsecond precision. Converts to UTC before saving to database. All output methods include timezone offset. Create via now(), fromISO(), fromSQL(), fromObject(), fromFormat(). Additional methods: setZone(), zoneName, offset.'
---
