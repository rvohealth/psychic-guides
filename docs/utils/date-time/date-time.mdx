...
Every parser on `DateTime` — `fromISO`, `fromSQL`, `fromFormat`, `fromObject` — throws `InvalidDateTime` on unparsable or calendar-impossible input rather than returning a value you can probe. There is no "invalid instance" to inspect: `DateTime.fromISO('garbage')` throws immediately, so a guard like `if (!DateTime.fromISO(v).toISODate()) { ... }` never reaches its check — the throw happens first. To validate untrusted input (an API param, an LLM-emitted value), parse it and catch the specific error instead:

`InvalidDateTime` is exported from `@rvoh/dream/errors`, not `@rvoh/dream`:

```ts
import { DateTime } from '@rvoh/dream'
import { InvalidDateTime } from '@rvoh/dream/errors'

function parseISODateTimeOrNull(v: string): DateTime | null {
  try {
...
Catch only the specific parse error you expect, not every error — an unguarded catch hides bugs that have nothing to do with parsing.
