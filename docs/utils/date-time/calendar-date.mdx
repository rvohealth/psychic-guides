...
CalendarDate throws `InvalidCalendarDate` errors for invalid operations. There is no "invalid instance" to inspect — every parser throws immediately on unparsable or calendar-impossible input, so catch the specific error rather than probing the result. `InvalidCalendarDate` is exported from `@rvoh/dream/errors`, not `@rvoh/dream`:

```ts
import { CalendarDate } from '@rvoh/dream'
import { InvalidCalendarDate } from '@rvoh/dream/errors'
