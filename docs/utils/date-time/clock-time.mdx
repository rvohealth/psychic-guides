...
ClockTime throws `InvalidClockTime` errors for invalid operations. There is no "invalid instance" to inspect — every parser throws immediately on unparsable input, so catch the specific error rather than probing the result. `InvalidClockTime` is exported from `@rvoh/dream/errors`, not `@rvoh/dream`:

```ts
import { ClockTime } from '@rvoh/dream'
import { InvalidClockTime } from '@rvoh/dream/errors'
