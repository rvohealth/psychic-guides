...
ai_summary: DateTime handles database timestamp columns. Automatically converted when loading models. Provides timezone manipulation, formatting, and math. Based on Luxon. Never use JavaScript's Date class in Psychic applications. Import from @rvoh/dream (not @types/luxon).
---
...
Database `timestamp` columns are automatically converted to DateTime objects when Dream models are loaded from the database. DateTime provides robust handling of datetimes, including timezone manipulation, formatting, and math. See [the Luxon documentation](https://moment.github.io/luxon/#/) for full details.
