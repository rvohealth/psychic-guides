...
Database `date` columns are automatically loaded into CalendarDate objects when querying Dream models. CalendarDate provides a clean way to deal with dates without worrying about timezone math (though several methods accept an optional `zone` option when setting the CalendarDate). The name CalendarDate was chosen because Date was already taken by Javascript's time class, and we wanted to emphasize that this class deals with days as the fundamental unit of time.
