...
:::info Psychic 3.0 (Koa-based)
This documentation is for **Psychic 3.0+**, which uses [Koa](https://koajs.com/) as its underlying HTTP framework.

If you're using **Psychic 2.0** (Express-based), please see the [Psychic 2.0 documentation](/docs/psychic-v2/README).

For migration guidance, see the [3.0.0 CHANGELOG](https://github.com/rvohealth/psychic/blob/main/CHANGELOG.md#300).
:::
...
Dream and Psychic have been in development since early 2023 and in production use at [RVOHealth](https://www.rvohealth.com/) (our employer and sponsor of the entire Psychic ecosystem, which consists of [8 npm packages](https://www.npmjs.com/settings/rvoh/packages), these guides, the Psychic RAG, and demo application) since December 2023. Inspired by over a decade of Ruby on Rails experience, the framework has evolved into a full-featured, batteries-included solution refined through real-world production use.

## What is Dream?

Dream is an in-house ORM designed to make interacting with your database a breeze. It follows the conventional [Active Record](https://en.wikipedia.org/wiki/Active_record_pattern) practices for modeling data, but provides a very powerful TypeScript-driven set of features to unleash powerful autocomplete mechanisms that make even the most dense applications possible to navigate, as well as robust association mechanisms to help clarify even the densest of domains.

Using decorators, you can expressively unleash powerful guards to restrain your data, or add hooks to modify data prior to saving.
...
Psychic is a fully-featured, TypeScript-driven web framework built on [Koa](https://koajs.com/). It leverages the MVC (Model, View, Controller) paradigm, providing the Dream ORM under the hood for data modeling. Philosophically, Psychic and Dream use opinionated conventions to enable seamless development, allowing beautiful type integrations to flow throughout your app with ease. By following a strict convention-over-configuration philosophy throughout our design, we enable you to write less while following best practice design concepts.

:::info Psychic 2.0 (Express-based)
If you're using Psychic 2.0 with Express, see the [Psychic 2.0 documentation](/docs/psychic-v2/README).
:::
...
Since the the OpenAPI decorator for the `index` method includes `cursorPaginate true`, the OpenAPI shape will reflect the `cursorPaginate` shape, with an array of results rendered using the "summary" serializer declared in the Place model's `serializers` getter. Alternatives to [cursorPaginate](/docs/openapi/pagination.mdx#scroll-pagination-cursor-based) for index endpoints are [paginate](/docs/openapi/pagination.mdx), for traditional pagination with page numbers (slower than `cursorPaginate` when results may include many records) and [many](/docs/openapi/controllers#many), for when pagination is unnecessary and you simply want to return an array of results.
...
const parsedDatetime = DateTime.fromISO('2024-01-15T13:17:44.123456Z')
const date = CalendarDate.today()
const parsedDate = CalendarDate.fromISO('2024-01-15')
```

Dream automatically converts database `timestamp` fields to `DateTime` objects and `date` fields to `CalendarDate` objects, providing built-in math, formatting, and timezone handling. see the [date and time](/docs/utils/date-time/overview) guides for full documentation.
:::
...
- [koa](https://koajs.com) (drives the underlying web server)
- [openapi](https://www.openapis.org) (drives integration with front end clients)
- [fast-json-stringify](https://www.npmjs.com/package/fast-json-stringify) (automatic fast JSON stringification based on OpenAPI derived schema compilation)
- [luxon](https://moment.github.io/luxon/#/?id=luxon) (Dream’s [DateTime](/docs/utils/date-time/date-time), [CalendarDate](/docs/utils/date-time/calendar-date), [ClockTime](/docs/utils/date-time/clock-time), and [ClockTimeTz](/docs/utils/date-time/clock-time-tz) leverage Luxon for date/time math)
