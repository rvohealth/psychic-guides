...
If you're using **Psychic 2.0** (Express-based), please see the [3.0.0 CHANGELOG](https://github.com/rvohealth/psychic/blob/main/CHANGELOG.md#300).
:::
...
export default class V1HostPlacesController extends AuthedController {
  public async create() {
    const place = await Place.create(this.extractParams(Place, ['name', 'style', 'sleeps']))
    this.created(place)
  }
...
- utilities like `extractParams` and `preloadFor`, which help you stay DRY when defining new associations on your serializers, or add or edit columns on your table.
...
- [luxon](https://moment.github.io/luxon/#/?id=luxon) (Dream's DateTime, CalendarDate, ClockTime, and ClockTimeTz leverage Luxon for date/time math)
