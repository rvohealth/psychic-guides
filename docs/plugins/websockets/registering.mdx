...
sidebar_position: 2
title: registering
ai_summary: "Register users with Ws class when websocket connections are established. Frontend provides auth token (encrypted user ID) via socket.io-client handshake. Backend decrypts token, finds user, and calls Ws.register(socket, user). Token generated during login flow."
---
