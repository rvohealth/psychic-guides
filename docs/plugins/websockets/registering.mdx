...
sidebar_position: 2
title: registering
ai_summary: "Register users with Ws class when websocket connections are established. Frontend provides auth token via socket.io-client handshake. Backend resolves the websocket user in the initializer and calls Ws.register(socket, user.id)."
---
...
Finally, in `src/conf/initializers/websockets.ts`, register your users with `Ws` upon connection via websocket:
...
`resolveWebsocketUser` is the boilerplate auth helper `create-psychic` scaffolds at `conf/system/resolveWebsocketUser.ts` — the login-endpoint-plus-`Encrypt.decrypt` pattern shown above is exactly what belongs inside its body. A newly-generated app rejects every websocket connection until that body is filled in with real auth logic, so no socket connects as an unauthenticated or misattributed user by default. See [Config](./config#auth-resolvewebsocketuser) for the helper's full shape.
