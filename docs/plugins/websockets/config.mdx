...
```bash
yarn add @rvohealth/psychic-websockets
```
...
import { PsychicApplicationWebsockets, Ws } from '@rvohealth/psychic-websockets'
...
3. Update your `initializePsychicApplication.ts` file to include the following:
...
The `ws:start` event is called whenever the psychic server is started. This allows you to establish socket bindings. Under the hood we are using [socket.io](https://socket.io) to power our websocket bindings, which means you can visit their documentation to understand more about setting up a websockets app within your application.
