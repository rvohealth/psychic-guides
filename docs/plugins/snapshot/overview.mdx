...
ai_summary: >-
  ...
  Recursively traverses HasMany and HasOne associations (including soft-deleted
  records), building JSON object of entire model tree. Skips BelongsTo and
  through associations (unless @SnapshotableFollowThrough used). Use
  @SnapshotableIgnore to filter columns or associations; required for
  required/passthrough on-clause associations (or takeSnapshot throws). Uses
  batched preloads to avoid N+1 up to 4 levels deep; self-referential trees
  supported. Recommended for background jobs.
---
...
Soft-deleted records are **included** in snapshots — the soft-delete default scope is removed so that deleted children appear in the output. This is intentional: for compliance use cases, the data existed and should be captured.
