...
title: testing
ai_summary: >-
  Backgrounded methods invoked immediately during tests by default. Set
  testInvocation to 'manual' to push jobs into BullMQ queues. Use
  WorkerTestUtils.clean() to reset Redis between tests. Use
  WorkerTestUtils.work() to manually work off jobs.
...
Always switch back to `automatic` in cleanup when you enable manual mode inside a single spec; otherwise later specs can silently stop running background jobs inline.
