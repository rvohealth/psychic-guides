...
This default applies to `schedule(...)`, `background(...)`, and `backgroundWithDelay(...)` alike — a spec that calls any of them executes the work immediately and synchronously, with the delay ignored, and no queue flush needed. The flip side: any environment guard inside the method (e.g. `if (serverEnvironment !== 'production') return`) also fires in tests, so a guarded method needs a `force`-style override to be exercised in a spec.
