...
ai_summary: >-
  ...
  with an incrementing attempt count, which the public entry method seeds. No
  parameter may carry a default, since every dispatch appends the BullMQ Job as
  a final argument. See BullMQ retry documentation for
  customization.
---
...
The `attempt` parameter is required rather than defaulted because every dispatch appends the BullMQ `Job` as a final argument — a defaulted trailing parameter receives that `Job` instead of its default. See [Services](./services#every-dispatch-appends-the-job-so-no-parameter-may-be-optional).
