...
3. rate-limiting certain types of work (when using named workstreams)
   :::
...
The `Job` parameter is optional and always comes last in the method signature — Psychic appends it to every dispatch, which is why no parameter of a backgrounded method may carry a default or a `?` (see [Services](/docs/plugins/workers/services#every-dispatch-appends-the-job-so-no-parameter-may-be-optional)). Job logs are accessible through the BullMQ dashboard and can be retrieved programmatically. Let unexpected errors throw so BullMQ can retry the job; catching and continuing marks the job successful.
...
- [backgroundWith](/docs/plugins/workers/backgroundWith) — running work once, after a delay, or at a one-off priority
