...
sidebar_position: 3
title: models
ai_summary: "Models extending ApplicationBackgroundedModel can call static and instance methods in background jobs. Use background('methodName', args) to background methods. For instance methods, Psychic stores primary key and looks up model when job executes. Prefer a backgrounded service for most use cases; reserve this for when the model itself needs to background its own methods."
---
...
Reach for this only when the model itself needs to background its own methods. For most use cases, prefer a [backgrounded service](./services) instead — it keeps the two-method (public entry point + `_`-prefixed implementation) pattern that most background work follows.
