---
sidebar_position: 1
ai_summary: "Loading and preloading associations for serialization. Must eager load all associations before serialization (no async in serializers). Use preloadFor to automatically load all associations required by a serializer. Use inspect:serialization CLI command to view serialization needs."
---
...
The string arguments passed to `preload`, `leftJoinPreload`, `innerJoin`, `leftJoin`, `load`, and `leftJoinLoad` form an **association chain** — a traversal path through related models. Multiple string arguments are **not** parallel associations; they describe a single path from one association to the next.

```ts
// Chain: User → posts → comments
// Loads posts on each user, AND comments on each post — NOT both posts and
// comments as separate associations on User.
const users = await User.preload('posts', 'comments').all()
users[0].posts[0].comments // [Comment{}, Comment{}, ...]
```

To load multiple unrelated associations, use separate calls instead — this loads `posts` and `pets` in parallel, not chained:
...
The last argument in a chain can be an array, branching the chain into multiple associations at the same level:
...
Chaining and parallel calls combine for more complex loading — two separate chains through `posts`:
...
`preload`, `leftJoinPreload`, `innerJoin`, `leftJoin`, `load`, and `leftJoinLoad` all accept `and`, `andNot`, and `andAny` condition objects placed after an association name in the chain. The condition applies to the association immediately before it:

```ts
// Condition on just the last association in the chain
const hosts = await Host.innerJoin('places', 'rooms', {
  and: { type: 'Bedroom' },
}).all()

// A condition on each association in the chain
const hosts = await Host.innerJoin(
  'places', { and: { style: 'cabin' } },
  'rooms', { and: { type: 'Bedroom' } },
).all()

// andAny follows whereAny semantics: each object is OR'd, keys within one
// object are AND'd
...
// andNot follows whereNot semantics
const users = await User.preload('posts', {
  andNot: { archived: true },
}).all()
```

Conditions attach to the association itself, not to the chain that reached it: when several chains reach the same association, they share a single condition node. A later chain that repeats a key (`and` / `andNot` / `andAny`) overwrites it, and a later chain carrying no conditions still inherits the earlier chain's. Set an association's conditions in one place.
...
```ts
// Room.place is a required BelongsTo (optional defaults to false).
...
Still allowed, because these can legitimately be empty or absent so a constraint can't violate a non-nullable contract:
...
To manually load all of these associations, you can use one of the eager loading methods, like `preload`, `leftJoinPreload`, `load`, or `leftJoinLoad`, depending on the situation. Here is an example of leveraging `preload` to eager load all of the associations specified.
...
Both of these statements are technically equivalent at this moment, but the first one is not only cluttered with preload statements that you otherwise couldn't care less about seeing, it is also only narrowly satisfying your serialization requirements.
...
<PackageManagerTabs
  code={`{{PM}} psy inspect:serialization Post`}
/>
...
<PackageManagerTabs
  code={`{{PM}} psy inspect:serialization Post summary`}
/>
