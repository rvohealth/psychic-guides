...
const user = await User
  .leftJoinPreload('posts', 'comments', ['images', 'videos'])
  .leftJoinPreload('posts', ['likes', 'similarPosts'])
  .firstOrFail()

// or tap into leftJoinPreloadFor to do this with ease!
const user = await User.leftJoinPreloadFor('detail').firstOrFail()
```

This additionally means that, when using leftJoinPreloadFor, your preload statements will automatically grow to accomodate new serializer `rendersOne`, `rendersMany`, or `delegatedAttribute` statements, preventing you from manually needing to maintain the loading logic, which can be a brittle process when done manually.
...
```ts
const users = await User.leftJoinPreloadFor('detail', (dreamClass, associationName) => {
