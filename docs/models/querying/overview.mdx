...
const users = await User.innerJoin('posts', {
  and: { content: ops.ilike('howyadoin') },
}).all()
```

:::tip
Always prefer Dream's built-in query and association APIs over dropping to raw SQL. Only reach for [`toKysely`](/docs/models/querying/toKysely) when the SQL you need genuinely isn't covered by Dream's public API, or as a final low-level step once a Dream query has traversed every association it needs.
:::
...
// Process records one by one, in batches (filter first with where)
await User.where({ active: true }).findEach((user) => {
...
const users = await User.innerJoin('posts', {
  and: { content: ops.ilike('tutorial') },
}).all()

// Chain through an association path: string arguments describe a single
// traversal (User → posts → comments), not two parallel joins — see
// "Association chaining" in the loading and preloading guide.
...
// Preload a nested association chain: User → posts → comments
const usersWithPostsAndComments = await User
  .preload('posts', 'comments')
  .all()

// Preload with a condition on the association
import { DateTime } from '@rvoh/dream'

const usersWithRecentPosts = await User
  .preload('posts', {
    and: { createdAt: ops.greaterThanOrEqualTo(DateTime.fromISO('2024-01-01T00:00:00Z')) },
  })
  .all()
```
...
// Left join with a condition on the association
const usersWithOptionalRecentPosts = await User
  .leftJoinPreload('posts', {
    and: { createdAt: ops.greaterThanOrEqualTo(DateTime.fromISO('2024-01-01T00:00:00Z')) },
  })
  .all()
```
...
Supplies the value an association's `and: { column: DreamConst.passthrough }` constraint needs at query time — most commonly locale for a data-driven i18n association.

```ts
// Post.currentLocalizedText requires a `locale` to be passed through
...
### Query-level write methods

These write directly from a query, without first loading records into application code — easy to miss, since most write examples in this guide start from an instance.

- [update guide](/docs/models/querying/update)
- [delete guide](/docs/models/querying/delete)
:::
