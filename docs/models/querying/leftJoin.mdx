...
Attach `and`, `andNot`, and `andAny` condition objects to the join. `andNot` and `andAny` follow `whereNot` / `whereAny` semantics, respectively:
...
const hosts = await Host.leftJoin('places', { andNot: { style: 'dump' } }).all()
```
