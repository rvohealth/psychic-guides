...
It can be tempting to reach for `all`, and then use plain TypeScript to further filter down your records, but if you have a very large users table, this approach can be very problematic for you. By leveraging query building mechanisms, we can inject conditions into the query before executing. Below, we use the `where` method to inject a WHERE clause into SQL, enabling us to filter down records at the SQL level before bringing them into memory:
