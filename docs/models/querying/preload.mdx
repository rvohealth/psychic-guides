---
ai_summary: "The preload method side-loads associations while executing a query. Executes queries in stages (separate SQL queries) rather than all at once. Supports arrays for multiple associations and chaining multiple preload calls. Cannot be mixed with leftJoinPreload in the same query."
---
# preload

Similar to [`leftJoinPreload`](/docs/models/querying/leftJoinPreload), the `preload` method enables you to side-load other associations while executing a query. The difference is that `preload` executes its queries in stages, rather than all at once. The query below will produce 3 SQL queries, one to load the users, one to load all of the users' posts, and one to load all of the posts' comments.

For loading associations on an already existing model instance, see [`load`](./load.mdx):
...
:::tip
Multiple string arguments describe a single traversal path (an **association chain**), not parallel associations — see [Association chaining](/docs/models/querying/loading-and-preloading#association-chaining) for the full concept, including conditions and the required-`BelongsTo` restriction.
:::

## Conditions

Place an `and`, `andNot`, or `andAny` condition object after an association name to constrain it:
...
If one needs to gain access to multiple resources, the last argument can always be an array:
