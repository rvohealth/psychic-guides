---
sidebar_position: 3
ai_summary: >-
  The numericality validator ensures a number is within a specified range. Use
  @deco.Validates('numericality', &#123; min: value, max: value &#125;)
  decorator to clamp values between min and max.
---

# numericality

Use the `numericality` validator to clamp a number between two values:

```ts
export default class Rating extends ApplicationModel {
  ...

  @deco.Validates('numericality', { min: 1, max: 5 })
  public rating: DreamColumn<Rating, 'rating'>
}
```
