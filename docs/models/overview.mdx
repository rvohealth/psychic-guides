...
Never give a column field an `=` initializer (e.g. `public status: DreamColumn<Place, 'status'> = 'draft'`). Dream serves column reads through prototype accessors and deletes any shadowing instance property after construction, so an initializer like that is silently discarded — no error, the default just never takes effect. Set the default at the database layer (`col.defaultTo(value).notNull()`) instead, and fall back to a `BeforeCreate`/`BeforeSave` hook only for a caller-dependent value the database can't express.

### Decorator setup

Each model creates its own `Decorators` instance, conventionally named `deco`:
...
Generators leave this import commented out on a freshly generated model that has no field-level `@deco.*` decorators yet, since an unused `Decorators` import would otherwise trip ESLint:
...
Uncomment it as soon as you add the model's first field-level decorator. Class-level decorators such as `@STI` and `@SoftDelete` are imported directly from `@rvoh/dream` and are unaffected by this rule — they don't require a `deco` instance.
...
- [locking guide](/docs/models/locking)
