...
ai_summary: "paramSafeColumns and paramUnsafeColumns work with extractParams / extractImplicitParams to limit allowed attributes. Default protection excludes primary key, timestamps, deleted_at, STI type, foreign keys. paramSafeColumns restricts to only specified columns. paramUnsafeColumns disallows specific columns. Restricted params automatically omitted from OpenAPI request body."
---

# paramSafeColumns / paramUnsafeColumns

:::caution
`paramSafeColumns` and `paramUnsafeColumns` _only_ provide protection when used with `PsychicController#extractParams` or `PsychicController#extractImplicitParams`.
...
export default class HostPlacesController extends HostBaseController {
  public async update() {
    ...
    // protected
    await place.update(this.extractParams(Place, ['name', 'description']))
    this.noContent()
  }
}
```
...
The `paramSafeColumns`/`paramUnsafeColumns` methods work in conjunction with `PsychicController#extractParams` and `PsychicController#extractImplicitParams` to limit what attributes are allowed to be set from params. This concept is similar to Rails' "strong params", but implemented at the model level and with different defaults.

## Default protection

Default protection is provided automatically: neither `extractParams` nor `extractImplicitParams` will ever include the following attributes (this cannot be overridden by `paramSafeColumns` or `paramUnsafeColumns`):
...
export default class HostPlacesRoomsController extends HostPlacesBaseController {
  ...
  public async update() {
    ...
    /* will not be included in the object returned by `this.extractImplicitParams(Room)`
    */
    await room.update(this.extractImplicitParams(Room))
    this.noContent()
  }
}
```

Restricted params can be added to the OpenAPI request body by specifying an `including` array. Note that this does _not_ alter the behavior of `extractParams` / `extractImplicitParams`, only the OpenAPI shape, so that requests may be allowed that include protected attributes; but these attributes will need to be handled explicitly:
...
export default class HostPlacesRoomsController extends HostPlacesBaseController {
  @OpenAPI(Room, {
    ...
    requestBody: {
       ...
       * returned by `extractParams` / `extractImplicitParams` (and we don't want it to be since simply setting the type would
       * not be operating on the correct model, even though the correct model would be hydrated
       * when loading from the database)
       */
      including: ['type'],
      // `extractParams` / `extractImplicitParams` will still strip `type` at runtime —
      // so the controller has to read it explicitly via `castParam` and act on it.
    },
  })
  public async create() {
