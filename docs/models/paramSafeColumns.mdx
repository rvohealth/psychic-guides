...
The `paramSafeColumns`/`paramUnsafeColumns` methods work in conjunction with `PsychicController#extractParams` to limit what attributes are allowed to be set from params. This concept is similar to Rails' "strong params", but implemented at the model level and with different defaults.

## Default protection

Default protection is provided automatically: `extractParams` will never include the following attributes (this cannot be overridden by `paramSafeColumns` or `paramUnsafeColumns`):
...
export default class HostPlacesRoomsController extends HostPlacesBaseController {
  ...
  public async update() {
    ...
    /* will not be included in the object returned by `this.extractParams(Room, [...])`
    */
    await room.update(this.extractParams(Room, ['name', 'capacity']))
    this.noContent()
  }
}
```

Restricted params can be added to the OpenAPI request body by specifying an `including` array. Note that this does _not_ alter the behavior of `extractParams`, only the OpenAPI shape, so that requests may be allowed that include protected attributes; but these attributes will need to be handled explicitly:
...
export default class HostPlacesRoomsController extends HostPlacesBaseController {
  @OpenAPI(Room, {
    ...
    requestBody: {
       ...
       * returned by `extractParams` (and we don't want it to be since simply setting the type would
       * not be operating on the correct model, even though the correct model would be hydrated
       * when loading from the database)
       */
      including: ['type'],
      // `extractParams` will still strip `type` at runtime —
