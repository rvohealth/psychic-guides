...
Soft deleted models can be included in queries by removing all default scopes or specifically the soft delete scope. Use `removeDefaultScope` when querying for a plurality (it preserves other default scopes) and `removeAllDefaultScopes` when targeting one specific record — see [removeDefaultScope and removeAllDefaultScopes](/docs/models/querying/removeDefaultScope) for the full rationale:
