...
Soft deleted models can be included in queries by removing the soft delete scope by name. `removeDefaultScope('dream:SoftDelete')` lifts that scope and nothing else, whether you're querying for one record or many — see [removing default scopes](/docs/models/querying/removeDefaultScope):
