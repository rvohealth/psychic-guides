---
sidebar_position: 4
ai_summary: 'Methods for updating records include save (after setting attributes) and update (single call with attributes object). Supports bulk updates via query builder. Triggers lifecycle hooks (BeforeUpdate, BeforeSave, AfterUpdate, AfterSave, etc.).'
---

import RoutesOutput from '@site/src/components/ui/routes-output'

# updating

To update a record that already exists, you can use a combination of attribute setters and the `save` method on your models to trigger updates:

```ts
import User from 'app/models/user'
const user = await User.first()

user.email = 'how@yadoin.biz'
await user.save()
```

## update

To achieve this in one line, you can call the `update` method:

```ts
await user.update({ email: 'how@yadoin.biz' })
```

## Bulk updates

Additionally, you can perform bulk updates like so:

```ts
await User.where({ email: ops.ilike('%chalupatowne%') }).update({ email: '' })
```

When the value you are writing depends on a value you just read, a plain update is not enough — see the [locking guide](/docs/models/locking) for guarded (compare-and-set) writes.
