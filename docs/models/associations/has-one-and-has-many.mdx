...
`dependent: 'destroy'` goes on the association that sees every child, never on one carrying an `and` condition. The cascade reaches exactly what that association's own definition matches, so where `Place` declares both `@deco.HasMany('Booking', { and: { status: 'confirmed' } })` and a plain `@deco.HasMany('Booking')`, it belongs on the plain one. Put it on the conditioned one and the remaining bookings stay live under a soft-deleted place, and a hard delete is refused outright by their `RESTRICT` foreign key, rolling the transaction back. The same holds for a `HasOne`: returning at most one record does not make an `and` condition harmless.

When the application owns child lifecycle, prefer `dependent: 'destroy'` on the association. With soft delete, keep database foreign keys on the default `restrict` behavior and let Dream run model hooks and soft-delete behavior:
...
Polymorphic relationships also need application-level dependent behavior, since the foreign key can point to many tables. Set the `dependent` option at the association level:
...
In the above example, any time a `Composition` or `Post` model is destroyed, dependent destroy applies to related ratings. If the parent is soft-deleted, every model in the dependent chain should also use `@SoftDelete()` so children are not permanently removed.
...
To skip a model's default scopes (such as `dream:SoftDelete`) when loading the association, pass the scope name(s):
...
The `and` option supports two special `DreamConst` values instead of a fixed value, both covered above under [And](#and):

- **`DreamConst.passthrough`** — the value is supplied at query time via `.passthrough({ column: value })`. Throws `MissingRequiredPassthroughForAssociationAndClause` if the value is `undefined` — pass `null` instead when there's genuinely no value.
- **`DreamConst.required`** — the value must be provided via `{ and: { column: value } }` conditions whenever the association is used in any query (`load`, `preload`, `innerJoin`, `leftJoin`, `associationQuery`, `association`, etc.). Throws `MissingRequiredAssociationAndClause` if not provided. Because the value can only be supplied for the association actually named in the query, an association carrying `DreamConst.required` cannot serve as an intermediate hop of a `through` chain — bridging across it always throws.

Both are required — the difference is *how* the value is supplied. Use `passthrough` when the value comes from broad query context (e.g. locale from a request header); use `required` when the value is specific to a particular association-loading call. **Neither can be combined with `dependent: 'destroy'`.**
