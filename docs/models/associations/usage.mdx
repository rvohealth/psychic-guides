...
**Pass ids when you don't hold an instance**: an array of ids for a bulk filter, a cross-type polymorphic batch load that falls back to explicit type/id groups, and background-job arguments, which must serialize to a queue and therefore pass ids that the job re-hydrates on the other end.
