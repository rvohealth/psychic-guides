---
sidebar_position: 6
ai_summary: "Polymorphic BelongsTo associations link one model to multiple other models using a foreign key and foreign key type. The polymorphic table stores both the foreign key (localizable_id) and the type (localizable_type) to reference different model types. Useful for shared associations like LocalizedText that can belong to multiple model types."
---
...
:::info
This example is a modified version of the domain model used in the [BearBnB tutorial](https://github.com/rvohealth/bearbnb).
:::
...
A single LocalizedText model can be used to represent LocalizedText for all of these different use cases (the names of the fields passed to the front end can be [customized during serialization](/docs/serializers/attributes#as)). The following entity relationship diagram (ERD) represents how each of these models is associated with LocalizedText:
