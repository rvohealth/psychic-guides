...
A single LocalizedText model can be used to represent LocalizedText for all of these different use cases (the names of the fields passed to the front end can be [customized during serialization](/docs/serializers/attributes#attributeas)). The following entity relationship diagram (ERD) represents how each of these models is associated with LocalizedText:
