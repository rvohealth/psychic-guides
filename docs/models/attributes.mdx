...
Usually, one sets attributes when [creating](./creating) or [updating](./updating), or directly using dot notation (`balloon.color = red` and then calling `balloon.save`). However, there are times (e.g. when adding custom getters and setters that transform the data prior to persisting) when it is helpful to bypass the getters and setters.
