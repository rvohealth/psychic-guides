...
sidebar_position: 3
title: hooks
ai_summary: 'Psychic and Dream expose lifecycle hooks. Psychic hooks (cli:sync, server:init:before-middleware, server:init:after-middleware, etc.) are configured in conf/app.ts or initializers. Dream hooks (model lifecycle hooks) are configured in conf/dream.ts. Hooks enable tapping into core internal events.'
---
...
:::tip
Reach for `PsychicApp.log(message, ...meta)` (or `PsychicApp.logWithLevel(level, message, ...meta)`) inside a hook callback rather than `console.log` — these route through the app's configured logger instead of writing unstructured strings straight to stdout.
:::
...
One interesting side-effect of the `cli:sync` hook is that you can actually append new types to the `types/psychic.ts` file by returning an object at the end of this callback:
...
When the Psychic web server is starting, this is the first hook to run. At this stage, Psychic has not processed any Koa middleware, nor has it processed your routes. It is the perfect spot to establish logging, or any other monitoring tools:
...
export default async (app: PsychicApp) => {
  app.on('server:init:after-middleware', (psychicServer: PsychicServer) => {
    // set up additional koa middleware
    psychicServer.koaApp.use(...)
  })
}
```
...
This hook is run while the web server is starting. It specifically runs after the `server:init:after-middleware` hooks have run, and after the `conf/routes.ts` file has been processed. At this stage, both psychic and koa will know about your application's routes.
...
export default async (app: PsychicApp) => {
  app.on('server:init:after-routes', (psychicServer: PsychicServer) => {
    // set up additional koa middleware, or do something else
    psychicServer.koaApp.use(...)
  })
}
```
...
The underlying [kysely](https://kysely.dev) engine we use to provide the database driver exposes a log lifecycle event that we can tap into during configuration. We expose that to you via the `db:log` event, enabling you to add custom logging for sql queries, like so:
...
<PackageManagerTabs code={`{{PM}} console`} />
