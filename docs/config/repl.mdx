...
`conf/repl.ts` is built for an interactive session — piping input into `pnpm console` races the boot sequence above and is unreliable. For scripted or agent-driven work against the database, write a standalone script that mirrors this file's boot order (load env, initialize the app, then import models) but skips the REPL-specific `DreamCLI.loadRepl(...)` call. See [CLI &rarr; repl](/docs/cli/repl#non-interactive-use) for the boilerplate.
