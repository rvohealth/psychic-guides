...
```ts
export default async function (dream: DreamApplication) {
  await dream.load('models', path.join(__dirname, '..', 'models'))
  await dream.load('serializers', path.join(__dirname, '..', 'serializers'))
  await dream.load('services', path.join(__dirname, '..', 'services'))
  ...
}
```
