...
The `ssl` field accepts `{ rejectUnauthorized: true }` (verified TLS, the default), `{ rejectUnauthorized: false }` (unverified TLS, for providers that don't serve a public-CA-signed certificate), or `false` (TLS off, for local development without TLS). The bare `useSsl` boolean shorthand is deprecated — use an explicit `ssl` value instead.
