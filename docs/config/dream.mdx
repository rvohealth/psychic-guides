---
sidebar_position: 2
ai_summary: 'Dream configuration is done in conf/dream.ts. Configures database connections (primary and replica), supports multiple database connections, primary key types, pagination page size, and other Dream-specific settings.'
---
...
In order for Dream to connect to your database, you need to provide it with explicit credentials. Dream supports both primary and replica DB connections, and configuration for both can be provided.

:::tip
Always use `AppEnv` (defined in `src/conf/AppEnv.ts`) to access environment variables — never `process.env` directly. `AppEnv` validates that required variables are present at boot time and produces a clear error message when one is missing. For variables that are only present in some environments, use `AppEnv.string(name, { optional: true })`, which returns `string | undefined` instead of throwing.
:::

```ts
export default async function (dreamApp: DreamApp) {
  // Use verified TLS by default; set DB_NO_SSL=true to disable (local dev only)
...
The `ssl` field accepts `{ rejectUnauthorized: true }` (verified TLS, the default), `{ rejectUnauthorized: false }` (unverified TLS, for providers that don't serve a public-CA-signed certificate), or `false` (TLS off, for local development without TLS).

Most managed Postgres providers (Supabase, Neon, Render, Azure Database for PostgreSQL Flexible Server) present a public-CA-signed certificate and work with the default. Providers that require a pinned CA certificate (AWS RDS, GCP Cloud SQL) need an additional `ca` field:
...
```ts
export default async function (dreamApp: DreamApp) {
  dreamApp.load('models', srcPath('app', 'models'), await importModels())
  dreamApp.load('serializers', srcPath('app', 'serializers'), await importSerializers())
  dreamApp.load('services', srcPath('app', 'services'), await importServices())
  ...
}
```
...
The primaryKeyType configuration informs the cli how to generate your migrations for you when building out models using the cli generators provided by dream. Valid values are `uuid7`, `uuid4`, `bigint`, and `integer` (this is also the same set of choices offered by `--primary-key-type` when provisioning a new app — see [installation](/docs/installation)).

```ts
export default async function (dreamApp: DreamApp) {
  dreamApp.set('primaryKeyType', 'uuid7')
  ...
}
```
...
Dream accepts a custom logger so you can integrate with your preferred logging solution. Configure it in `conf/dream.ts`:
...
:::warning
Never use `console.log` in application code. `console.log` is blocking, bypasses the configured logger, and pollutes production logs with unstructured strings. Use `PsychicApp.log(message, ...meta)` for general output and `PsychicApp.logWithLevel(level, message, ...meta)` when you need a specific level (`'debug' | 'info' | 'warn' | 'error'`). These route through the configured logger so output respects log level, transports, and structured format. REPL / `psy console` sessions are exempt — interactive output to stdout is the point. If no logger is configured, Dream falls back to `console.log`, which is why configuring one is recommended.
:::
...
```ts
export default async function (dreamApp: DreamApp) {
  dreamApp.set('parallelTests', AppEnv.integer('DREAM_PARALLEL_TESTS', { optional: true }) || 1)
  ...
}
```
