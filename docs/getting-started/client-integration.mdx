...
export default class UsersAPI {
  ...
  public static signin(body: Partial<User>) {
    return apiCall('api.v1.signin.POST').send({ body })
  }

  public static me(body: Partial<User>) {
    return apiCall('api.v1.users.me.GET').send({ body })
  }
}
```
...
describe('visitor visits the signup page', () => {
  it('allows visitor to fill form and sign up, and then use that form to log in', async () => {
    await visit('/signup')
