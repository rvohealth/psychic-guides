...
- **Boundary-level input bounding.** `castParam`/`extractParams` accept `minLength`/`maxLength` on strings and `minimum`/`maximum` on numeric types, checked after type coercion and applied element-wise on array casts. Use these to bound untrusted input at the controller boundary rather than validating size later in a model or service — see [Params — Bounding length and range](/docs/controllers/params#bounding-length-and-range).
...
- **Database TLS.** The boilerplate ships `ssl: { rejectUnauthorized: true }` (verified TLS via Node's system CA store), which works out of the box with managed providers that present a public-CA-signed certificate (Supabase, Neon, Render, Azure Database for PostgreSQL Flexible Server). For providers that present a private-CA certificate (AWS RDS, GCP Cloud SQL), add a `ca` bundle: `ssl: { rejectUnauthorized: true, ca: readFileSync('/path/to/ca.pem') }`. For providers that present a self-signed certificate (Heroku Hobby, some local Docker images), set `ssl: { rejectUnauthorized: false }` — encrypted but **not** authenticated.
...
Koa middleware that runs outside the controller layer (e.g. gating a Bull Board dashboard) can't call `this.getCookie()` and has to decrypt with `Encrypt.decrypt` directly — including deciding how to treat `DecryptionError` versus `DecryptionRotationError`. See [Cookies — Decrypting cookies outside a controller](/docs/controllers/cookies#decrypting-cookies-outside-a-controller) for the full error-handling contract; the short version is: log-and-treat-as-unauthenticated for a single bad cookie, but rethrow if both the current and legacy keys fail, since that signals a broken rotation rather than a forged cookie.
