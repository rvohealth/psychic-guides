...
ai_summary: "App-developer security guide for Psychic + Dream. Index for the per-topic guides (rate limiting, output encoding, workers and Redis, supply chain) plus a single-page checklist of every security knob the framework exposes — params, redirects, sessions, encryption, file uploads, websockets, OpenAPI, and the application-layer auth patterns the framework expects you to provide."
---
...
- **Mass-assignment safety.** `extractParams(Model, [...])` is a type-checked, runtime-intersected primitive; the legacy `paramsFor` is `@deprecated` to nudge migration.
...
- **Database TLS.** Set `useSsl: true` in your Dream config to enable Postgres TLS. **Caveat (tracker R-027):** today the framework's built-in TLS config sets `rejectUnauthorized: false`, so the connection is encrypted but the server certificate is *not* validated against the system CA store. This is opportunistic TLS, not authenticated TLS — fine on a private VPC where the network is the trust boundary, not fine over the public internet. A future Dream release will accept a full TLS config (CA bundle + `rejectUnauthorized`) on `SingleDbCredential`; until then, deployments needing verified Postgres TLS should run on a network where MITM is not in the threat model.
...
| Mass assignment | `extractParams(Model, [allowed])` / `extractImplicitParams(Model)` | required for new generators | [Mass assignment below](#mass-assignment) |
...
```ts
import { extractParams } from '@rvoh/psychic'
import User from '../models/User'

class UsersController extends AuthedController {
  async update() {
    const allowed = extractParams(User, ['email', 'name', 'avatarUrl'], { params: this.params })
...
Generators emit this scaffold for non-admin namespaces. Admin-namespace generators emit `extractImplicitParams(Model)` because the model-level `paramSafeColumns` declaration is the canonical allowlist. Override at scaffold time with `--with-extract-params` / `--with-extract-implicit-params`. The legacy `paramsFor` still works but is `@deprecated` — IDEs strike through every call site.
