...
ai_summary: >-
  Psychic's posture on transitive-dependency advisories: distinguish runtime (in
  your published bundle) from dev/build-time (test runner, linters, codegen,
  scaffolding tools), and only the first category is a real production risk.
  Includes the override recipes for npm / yarn / pnpm and the current snapshot
  of advisories worth pinning.
