...
Serializers and the `@OpenAPI` decorator also support hand-written specs when a shape is genuinely ad hoc. For model-shaped data, let Psychic derive the schema from the model and serializer: model responses should use serializers, model request bodies should use `requestBody.params` or `requestBody.including`, and computed response objects should use `ObjectSerializer`.
