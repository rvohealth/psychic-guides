...
Here, `bedTypes` is an array of enum values (e.g., `['cot', 'bunk']`). The `BedTypeSerializer` transforms each one into an object with both the raw `value` and a localized `label`. Passthrough data from the parent is automatically forwarded — no extra wiring needed.
