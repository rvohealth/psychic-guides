...
interface UserViewModel { id: string; name: string; }
...
const UserViewModelSerializer = (user: UserViewModel) =>
  ObjectSerializer(user).attribute('id', {openapi: 'string'}).attribute('name', {openapi: 'string'});
...
const PetViewModelSerializerWithUser = (pet: PetViewModel) =>
  ObjectSerializer(pet)
    ...
    .rendersOne('owner', UserViewModelSerializer, { // Serializer function provided
...
interface PetViewModel { id: string; name: string; species: string; }
...
const PetViewModelSerializer = (pet: PetViewModel) =>
  ObjectSerializer(pet).attribute('id', {openapi: 'string'}).attribute('name', {openapi: 'string'});

const UserViewModelSerializerWithPets = (user: UserViewModel) =>
  ObjectSerializer(user)
    .attribute('id', {openapi: 'string'})
    .attribute('name', {openapi: 'string'})
    .rendersMany('animals', PetViewModelSerializer, { // Serializer function provided
...
## Preloading Data (Crucial for `DreamSerializer`)

When using `DreamSerializer` with associations, always preload the associated data in your controller or data-fetching logic to prevent N+1 query problems.

```typescript
// In your controller:
const user = await User.preload('pets').find(userId);
// Now, UserSerializerWithPets(user) can efficiently access user.pets.
```
