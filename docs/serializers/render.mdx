...
### Using Implicit Rendering in Controllers
...
### Passthrough Data in Nested Serializers

When using the `serializer` option with `rendersOne` or `rendersMany`, passthrough data from the parent serializer is automatically passed to the nested serializer function:

```ts
// Nested serializer receives passthrough automatically
export const BedTypeSerializer = (bedType: BedTypesEnum, passthrough: { locale: LocalesEnum }) =>
  ObjectSerializer({ bedType }, passthrough)
    .attribute('bedType', { as: 'value' })
    .customAttribute('label', () => i18n(passthrough.locale, `rooms.Bedroom.bedTypes.${bedType}`), {
      openapi: 'string',
    })

// Parent serializer passes passthrough to nested serializers
export const RoomBedroomSerializer = (room: Bedroom, passthrough: { locale: LocalesEnum }) =>
  DreamSerializer(Bedroom, room, passthrough)
    .attribute('id')
    .rendersMany('bedTypes', { serializer: BedTypeSerializer })
// passthrough is automatically passed to BedTypeSerializer for each bedType
```
