...
import { getLocaleFromHeaders } from 'app/helpers/i18n_helpers';
...
export default class ApplicationController extends PsychicController {
  ...
  public configureSerializers() {
    ...
    this.serializerPassthrough({ locale: getLocaleFromHeaders(this.req.headers) });
  }
}
```
...
export const MyDataSerializer = (data: MyData, options?: SerializerOptions) =>
  ObjectSerializer(data, options) // Pass options to ObjectSerializer
    ...
    .customAttribute('localizedGreeting', (d, opts) => { // `opts` here contains the passthrough data
      if (opts?.locale === 'es') {
