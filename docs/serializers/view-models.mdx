...
3.  **Needing Asynchronous Operations for Data Preparation:**
    Dream serializers are **intentionally synchronous** (see [Overview](./overview.mdx#synchronous-by-design) for more details). This design choice helps prevent accidental N+1 query problems or other side effects during the serialization phase itself, which should be a straightforward data transformation step.
