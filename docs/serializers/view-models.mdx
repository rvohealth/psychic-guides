...
ai_summary: ViewModels are intermediate structures for shaping data before serialization. Use when combining multiple models, complex transformations, or needing async operations. Serialize with ObjectSerializer (requires explicit OpenAPI shapes). Keeps data preparation logic separate from serialization.
---
