...
const UserViewModelSerializer = (user: UserViewModel) =>
  ObjectSerializer(user)
    .attribute('id', { openapi: { type: 'string', description: 'The unique identifier' } })
    .attribute('email', { openapi: { type: 'string', format: 'email' } });

// Example usage:
const userInstance: UserViewModel = { id: '123', email: 'test@example.com', name: 'Test User' };
...
const UserSerializer = (user: User) =>
  DreamSerializer(User, user)
    ...
    .attribute('lastLoginAt', { openapi: { description: 'Timestamp of the last login', format: 'date-time' } }); // DB type inferred, but adding description and format
```
...
const UserWithVirtualSerializer = (user: User) =>
  DreamSerializer(User, user)
    ...
    .attribute('fullName', { openapi: { type: 'string', description: 'The full name of the user.' } }); // `openapi` required for virtual attribute
```
...
const UserSerializerWithAlias = (user: User) =>
  DreamSerializer(User, user)
    .attribute('email', { as: 'userEmail' }); // Type inferred if 'email' is a DB column
...
const UserSerializerWithCustom = (user: User) =>
  DreamSerializer(User, user)
    ...
    .customAttribute('initials', () => `${user.firstName?.[0] ?? ''}${user.lastName?.[0] ?? ''}`.toUpperCase(), {
...
const PetWithUserEmailSerializer = (pet: Pet) =>
  DreamSerializer(Pet, pet)
    ...
    .delegatedAttribute('user', 'email', { openapi: { type: 'string', format: 'email', description: "Owner's email" } });
...
const JsonDemoSerializer = (data: ModelWithJson) =>
  DreamSerializer(ModelWithJson, data)
    .attribute('id') // Inferred
    .jsonAttribute('settings', { // `settings` is a text column in DB
      openapi: { 
        type: 'object', 
        properties: { 
          theme: { type: 'string' }, 
