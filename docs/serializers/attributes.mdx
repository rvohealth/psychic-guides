...
title: attributes
ai_summary: >-
  Serializer attribute methods include attribute (with options: openapi,
  default, as, precision), delegatedAttribute (renders from nested
  objects/associations), and customAttribute (arbitrary transformation with
  flatten option). All support OpenAPI shape definitions.
---
...
`customAttribute` is for values computed from the model's own columns and passthrough data — a localized label, a formatted string, and so on. It is not discovered by [`preloadFor`](/docs/models/querying/preloadFor): `preloadFor` doesn't inspect the callback body, so if a `customAttribute` function reads an association (including a fallback like `() => current?.title ?? fallback.title`), that association won't be preloaded automatically, and rendering throws `NonLoadedAssociation`. Treat that error as the signal to reach for [`delegatedAttribute`](#delegatedattribute) or a flattened `rendersOne` instead — both are discovered by `preloadFor`, so the association is loaded without any extra controller wiring. See [layering two delegatedAttributes](#layering-two-delegatedattributes-onto-the-same-key) for the fallback-value case specifically.
