...
npx @rvoh/create-psychic new howyadoin
```
...
Every provisioning question can also be answered up front with a flag, skipping the interactive prompts entirely. If any option is omitted, `create-psychic` falls back to interactive mode to ask for it. Boolean flags can be negated with `--no-` (e.g. `--no-workers`).
...
The two skill flags install [psychic-skill](https://github.com/daniel-nelson/psychic-skill), a Dream and Psychic skill for AI coding agents.
...
Though the app provisioner is driven by `npx`, new Psychic applications can choose `pnpm`, `yarn`, `npm`, or `bun` as their package manager. The examples in these docs use package-manager tabs where command syntax differs. For non-tabbed examples, substitute your project's package manager: `pnpm psy`, `yarn psy`, `npm run psy`, or `bun run psy`.
