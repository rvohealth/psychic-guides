---
sidebar_position: 4
ai_summary: "Psychic automatically handles certain errors with intuitive response codes. castParam raises 400 for invalid params. extractParams / extractImplicitParams raise 400 for invalid model attributes. Failable lookups (findOrFail, firstOrFail) raise 404 for not found. Validation errors raise 422."
---
...
Similar to `castParam`, `extractParams` (and `extractImplicitParams`) will raise a 400 exception if any of the params provided matching one of the model attributes fails type validation:
