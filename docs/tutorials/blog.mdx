...
ai_summary: "Tutorial for building a blog application with Psychic. Covers provisioning, scaffolding, user signup, posts, and full spec coverage including frontend feature specs. Step-by-step guide from app creation to completion."
---
...
The first step to building the app is to provision a new Psychic application. To do this, we will use the app provisioning tool provided by psychic. Before this is done, let's make sure we are using a modern version of nodejs, and have the required dependencies installed.
...
With a modern node version and a valid version of postgres installed, we are ready to provision a new psychic application. Run the following in the terminal (from the directory of your choice) to provision a new Psychic app
...
<PackageManagerTabs
  code={`NODE_ENV=test {{PM}} psy db:create
NODE_ENV=development {{PM}} psy db:create`}
/>
...
Since we are creating a blog, we are going to need at minimum a users and blogs table in the database, so that we can allow users to sign up, sign in, and create new blog posts. To get started, let's generate the user model and begin building out the scaffolding for authentication.

<PackageManagerTabs
  code={`{{PM}} psy g:model User email:string passwordDigest:string`}
/>
...
Lucky for us, running migrations will automatically re-sync these types, so let's start by making some tweaks to the migration file. Let's open the migration file and examine its contents:
...
<PackageManagerTabs
  code={`NODE_ENV=test {{PM}} psy db:migrate
NODE_ENV=development {{PM}} psy db:migrate`}
/>

With the migrations run, you can re-open the `src/app/models/User.ts` file, and all type errors should now be cleared.

this is a pretty good spot to make the first commit:
...
Next, we want to add a `password` field, which, when updated, will be converted to a hashed password field in the database. To do this, we can utilize the `@deco.Virtual` decorator to add a virtual column to the model. Virtual columns are fields that act like columns, but do not persist to the actual database.

To do this, let's first add argon2 as a dependency. We will use this package to generate one-way hashes of user passwords.

<PackageManagerTabs
  code={`{{PM}} add argon2`}
/>

With the package added, let's first write a spec to cover the new behavior for the virtual column:
...
This spec should already contain type errors, since the user model does not yet have a virtual password field. Let's go ahead and run the spec anyway to make sure we get a failure. Once we see a failure, let's go and add implementation to solve for this spec.
...
with the new Virtual decorator applied, we can now run `sync` to rebuild types for the app:

<PackageManagerTabs
  code={`{{PM}} psy sync`}
/>

With the new virtual attribute in place, re-running the specs should produce a successful run:
...
Let's also update the `createUser` factory to automatically supply a password field, so that we don't always have to remember to provide one when we are creating users for specs.
...
<PackageManagerTabs
  code={`{{PM}} uspec spec/unit/models/User.spec.ts`}
/>
...
Now, let's add implementation to get the spec passing:
...
Now that we have a user with a functioning password field, we can begin wiring up a client flow to create new users in the application. Let's start by adding a feature spec:
...
Running the feature spec, we should get failures:

<PackageManagerTabs
  code={`{{PM}} fspec spec/features/signup.spec.ts`}
/>
...
Let's proceed by beginning to modify the client react application, to add routing and links for the sign up page. To do so, we will start by changing into the client directory and adding the `react-router` dependency, since vite and react do not ship by default with routing mechanisms. If you have chosen to use Nextjs, or another client that provides routing, you can skip this.
...
With react-router installed, let's configure react router in the `client/src/main.tsx` file.
...
With these pages in place, we can run the feature spec again to get a new failure:
...
<PackageManagerTabs
  code={`{{PM}} psy g:controller AuthSessionController`}
/>
...
import { paths } from '../../../src/types/openapi/tests.openapi.js'
...
To solve this error, let's add the implementation for sign up. To start with, we will need to add a route to the routes file:
...
Next, let's add the implementation for the sign-up endpoint in the new controller:
...
With this controller in place, we can now run `{{PM}} psy sync` to update the types to acknowledge the new endpoint.

<PackageManagerTabs
  code={`{{PM}} psy sync`}
/>

With types synced, we should now see all errors disappear in the spec file, since it is now aware of the new endpoint we added. Running the spec again, we should see the endpoint tests passing.
...
With the controller specs passing, we can work our way back to the client to complete the sign up spec. Before we do this, however, we can tap into special openapi integrations to automatically build an api bridge between the backend and client.

Let's start by adding redux to the client project dependencies.
...
<PackageManagerTabs
  code={`{{PM}} add @reduxjs/toolkit react-redux`}
/>
...
With redux installed, we can begin integrating into the client application. We will need to start with a provider, like so:
...
Next, let's add the app reducer, which we will use to keep track of the user's authenticated state:
...
Now that we have redux integrated into the application, let's add an openapi-redux bridge to enable us to sync the backend openapi to redux whenever the openapi document is synced. To do this, let's use the `setup:sync:openapi-redux` cli command, like so:
...
The output from the command we just ran is also alerting us to another set of updates we need to make to the redux configuration. First, let's run sync:

<PackageManagerTabs
  code={`{{PM}} psy sync`}
/>

Now, let's make the changes to the store.ts file, which were requested in the CLI output of the redux setup:
...
With this all in place, we should be ready to start utilizing the redux bindings within the signup page.
...
This will set us up to call to the backend and sign up the user. Let's also set up a SigninPage for the app to redirect to after signing up:
...
Now, let's wire up the SigninPage into the app within the `App.tsx` file:
...
With signup working, we are ready to implement the sign in strategy. There is no one-size-fits-all solution to this, so I will go with a very simnple authentication strategy here to keep this tutorial simple. In the modern era, a basic username-password authentication scheme is outdated, so I would recommend swapping this strategy for something more sophisticated.

To start with, let's write a feature spec to capture the behavior of the login flow.
...
describe('Signin', () => {
  it('allows user to sign in', async () => {
    await createUser({ email: 'my@email', password: 'password123' })
    await visit('/')
...
Running it again, we will watch the error move forward. Now it is complaining because there is no "Dashboard" text on the page. The dashboard text is kind of a placeholder, since we don't totally know what we will show when the user logs in yet. We will eventually make a dashboard page that contains that missing text to pass the test. First, however, we will need to complete the sign in endpoint, so that we have somewhere to send the login data from the inputs.

### Sign in endpoint

To build the signin endpoint, let's add a method to the `AuthSessionController` to receive the signin and process it. Let's start with a controller spec to cover the behavior:
...
// api/spec/unit/controllers/AuthSessionController.spec.ts
  ...
  describe('POST sign-in', () => {
    ...
    context('with an email address that is not in the system', () => {
...
Immediately, we should see errors in this file, since the OpenapiSpecRequest class we are leveraging does not recognize this `sign-in` endpoint yet. Running the test, we get an error saying it expected 204, but got a 404. Let's add the endpoint to the controller now to fix this, as well as the type errors:
...
Additionally, let's add a corresponding route to the routes file:
...
Now we can run the endpoint test again, and it should be passing.
...
```sh
 ✓ spec/unit/controllers/AuthSessionController.spec.ts (5 tests) 920ms
   ...
   ✓ AuthSessionController > POST sign-in > with an email address that is not in the system > blocks sign in 91ms
...
Let's add a new endpoint for determining if the user _is_ authenticated. You may or may not need this endpoint for your app, but it will be helpful for this basic app, since it can easily be a switch to use to determine if the user is authenticated or not.
...
Running this spec should give us a 404. We can fix that by adding both a route and matching action on the controller:
...
The routes file will show a type error because the `status` arg, which will be cleared once we add the method to the AuthSessionController. Let's add that now.
...
let's sync, since we have added a new endpoint, then check the controller spec again. The errors will have cleared now.

<PackageManagerTabs
  code={`{{PM}} psy sync`}
/>

Running the controller spec again, we got all successes:
...
```sh
 ✓ spec/unit/controllers/AuthSessionController.spec.ts (7 tests) 1300ms
   ...
   ✓ AuthSessionController > POST sign-in > with an email address that is not in the system > blocks sign in 147ms
...
This should be enough to get going on the sign in flow for the front end. Let's start by bringing in the new signin endpoint to the SigninPage component, so we can submit the data. Let's update the signin page:
    ...
    <div>
      ...
      <button
        onClick={async () => {
          ...
          // brute navigation to help specs along, otherwise would use navigate('/')
...
Next, we will tap into the `GET /auth/status` endpoint I mentioned before to determine if the user is authenticated or not within the app.tsx file, and use it to conditionally render the Dashboard when the user is authenticated.
...
With these files in place, we can run the signin feature spec again and it should pass
...
Next, we will set up the scaffolding for the blog. To do anything, we will need to start by generating a Post model. This model will belong to a User, and it will have a title and a body. We can now tap into psychic's resource generator to make this very trivial for ourselves.

<PackageManagerTabs
  code={`{{PM}} psy g:resource api/v1/posts Post user:belongs_to title:text body:text`}
/>

This automatically generates a controller, model, serializer, controller spec, and model spec. It additionally adds the necessary route entry to build this route in the routes file. Opening the `api/spec/unit/controllers/Api/V1/PostsController.spec.ts` spec, you will find it has basically exactly what we need to test the new resource. Here is a copy of that file, for reference:
...
Let's first run the migrations to capture the new model
...
Examining the spec file now, most type errors will have gone away, but a few remain. Let's fix them by adding the reverse association to the User model.
...
Now let's run sync again to sync the new associations to the type layer.
...
<PackageManagerTabs
  code={`{{PM}} uspec spec/unit/controllers/Api/V1/PostsController.spec.ts`}
/>
...
export default class ApiV1PostsController extends ApiV1BaseController {
  ...
  public async create() {
    let post = await this.currentUser.createAssociation('posts', this.extractParams(Post, ['title', 'body']))
    if (post.isPersisted) post = await post.loadFor('default').execute()
    this.created(post)
  }
  ...
  public async update() {
    const post = await this.post()
    await post.update(this.extractParams(Post, ['title', 'body']))
    this.noContent()
  }
...
With the implementations uncommented, we can immediately see that we will be wrangling with how to set "currentUser" on the controller, so we'll get to that soon. Let's run the spec first:
...
let's solve for this error now, by updating the AuthedController. Uncommenting a few lines and simplifying the authedUserId, we end up with this:
...
Now, we can run the PostsController spec again and find it passing!
...
This makes life much easier. Now, we can simply run sync to make sure the front end openapi types are totally compiled, and then we can begin to write feature specs, which will now be much easier to solve, since the Post endpoints are already built, and their redux bindings are already available.
...
Before we begin, let's abstract some of the sign in logic to a helper, so we can reuse it going forward:
...
Now, let's use this spec helper in the index spec:
...
export default function PostsIndexPage() {
  const { data } = useGetApiV1PostsQuery({ cursor: undefined })
...
Revisiting the `PostsIndex`, we should see the type error has been vanquished! Psychic is intelligent about syncing serializer shapes down through the openapi decorators on your controller, which informs the redux bindings that sync to your client, keeping the whole process pretty much hands-off for you.

Next, add the PostsIndexPage to the `App.tsx` file:
...
With the PostsIndex in place, let's re-run the spec, which is now green!
...
```ts
// spec/features/posts/show.spec.ts
...
Now, with the index in place, let's run the spec again:
...
```ts
// client/src/pages/authed/posts/PostEditPage.tsx
...
And then make sure our post edit page is included in the application routes:
...
With this in place, the specs will pass:
