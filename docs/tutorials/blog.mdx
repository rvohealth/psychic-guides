...
```sh
pnpm psy g:model User email:string passwordDigest:string
```
...
```sh
pnpm fspec spec/features/signup.spec.ts
...
```sh
pnpm psy g:controller AuthSessionController
```
...
With this controller in place, we can now run `pnpm psy sync` to update our types to acknowledge the new endpoint.
...
```sh
pnpm psy setup:sync:openapi-redux
```
...
```sh
pnpm fspec:visible spec/features/signin.spec.ts
```
...
```sh
pnpm psy g:resource api/v1/posts Post user:belongs_to title:text body:text
```
...
```sh
pnpm uspec spec/unit/controllers/Api/V1/PostsController.spec.ts
```
