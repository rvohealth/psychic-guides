...
title: File Uploads
ai_summary: >-
  Recommended patterns for handling file uploads in a Psychic app, leading with
  presigned-URL object-storage flows and covering the fallback local-handling
  recipe.
...
const s3 = new S3Client({ region: AppEnv.string('AWS_REGION') })
  ...
  public async presign() {
    ...
    const url = await getSignedUrl(
      ...
      new PutObjectCommand({
        Bucket: AppEnv.string('UPLOADS_BUCKET'),
        Key: key,
    ...
    this.created({
      url,
      key,
      expiresAt: DateTime.now().plus({ seconds: 60 }).toISO(),
    })
  }
}
```
