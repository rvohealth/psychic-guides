...
Taking advantage of one of the many decorators provided by dream, we can leverage the `@deco.Virtual` decorator to indicate to dream that a field should behave, in principle, like a database column, even though it is not in the database. Using clever abstractions, you can take advantage of virtual attribtues in a miriad of ways.
