...
+    },
   ...
   public async create() {
-    // let room = await this.currentPlace.createAssociation('rooms', this.extractParams(Room,
-    //   ['position'],
-    // ))
...
+
+    // extractParams is based on the table (even virtual attributes are associated with
+    // the table at the type level), so passing the STI children to extractParams
+    // would not alter the results
+    const roomParams = this.extractParams(Room, ['position'])
+
+    switch (roomType) {
+      case 'Bathroom':
...
@@ -50,9 +105,9 @@ export default class V1HostPlacesRoomsController extends V1HostPlacesBaseControl
   ...
   public async update() {
-    // const room = await this.room()
-    // await room.update(this.extractParams(Room,
-    //   ['position'],
-    // ))
-    // this.noContent()
+    const room = await this.room()
+    await room.update(this.extractParams(Room, ['position']))
+    this.noContent()
   }
