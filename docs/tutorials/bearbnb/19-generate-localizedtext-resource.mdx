...
id: bearbnb-19-generate-localizedtext-resource
---
...
Leverages `--only=update,destroy` to only generate the update and destroy actions. Each polymorphically associated model's controller will have an action for creating an associated LocalizedText model, and Host-facing serializers can display LocalizedTexts, but the actions for maintaining all LocalizedText will be centralized in the LocalizedText controller.
``````
...
diff --git a/api/src/db/migrations/1782836308128-create-localized-text.ts b/api/src/db/migrations/1782836308128-create-localized-text.ts
new file mode 100644
index 0000000..978878c
--- /dev/null
+++ b/api/src/db/migrations/1782836308128-create-localized-text.ts
@@ -0,0 +1,46 @@
