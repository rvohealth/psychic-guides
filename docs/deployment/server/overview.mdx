...
ai_summary: >-
  Deployment requirements: PostgreSQL database, Node.js server instance, load
  balancer, secure secret provisioning. Additional: Redis for
  websockets/workers, worker instances for background jobs, console instance for
  console access. See AWS guides for provisioning details.
---
...
Not all of these things are necessarily the responsibility of these guides to explain, so these guides will instead focus narrowly on how to provision instances in a given environment, and how to launch respective processes, but in general, if you just follow tutorials for setting up a Koa server with postgres, it will get you most of the way there.

:::tip

- see the [aws](/docs/deployment/server/aws) guides to learn how to provision your application for aws.
