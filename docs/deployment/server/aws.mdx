...
title: aws
ai_summary: >-
  Deploy to AWS using Fargate. Single Dockerfile provisions web server, worker,
  and console instances. Different initialization commands for worker, console,
  or web. Requires ECR instances for each. Includes Dockerfile example and
...
To deploy your application to AWS, we will be leveraging AWS Fargate to provision the web server, worker, and console instances. We will be using a single Dockerfile to provision all of them, but will run different initialization commands depending on if the instance provisioned is a `worker`, `console`, or `web` instance. For now, this guide assumes you have the following already set up:
...
ENV APP_NAME=YOUR_APP_NAME_HERE
RUN apk -U upgrade
RUN apk add dumb-init
ARG INSTALL_PG_DUMP_RESTORE
RUN if [ "$INSTALL_PG_DUMP_RESTORE" = '1' ] ; then apk add postgresql ; fi
...
RUN corepack enable --install-directory ${COREPACK_HOME} && corepack enable pnpm && corepack prepare pnpm@${PNPM_VERSION} --activate && corepack use pnpm@${PNPM_VERSION}
...
# add setup here if you plan on deploying a console instance and want extra utils to be set up for it
FROM production AS console
USER root
RUN apk add screen=5.0.1-r0 util-linux=2.41.2-r0 su-exec=0.3-r0
```
...
if [ $WEB_SERVICE ]; then
  TMP=/dev/shm NODE_ENV=production node ./dist/src/conf/system/cli.js db:migrate --skip-sync && \
  TMP=/dev/shm NODE_ENV=production node ./dist/src/conf/system/cli.js db:seed && \
  ./node_modules/.bin/pm2-runtime start ./api.ecosystem.config.cjs --env production
elif [ $WORKER_SERVICE ]; then
  ./node_modules/.bin/pm2-runtime start ./worker.ecosystem.config.cjs --env production
...
And the pm2 ecosystem files as well:
  ...
  apps: [
    {
      ...
      kill_timeout: 5000, // Give process 5 seconds to shut down gracefully
      ...
      // max_memory_restart: '500M',  // Only uncomment if you want memory-based restarts
...
and the worker ecosystem file (if using workers)
