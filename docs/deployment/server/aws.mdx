...
RUN mkdir -p /home/node/.pnpm && chown node:node /home/node/.pnpm
...
RUN corepack enable --install-directory ${COREPACK_HOME} && corepack prepare pnpm@${PNPM_VERSION}
...
<PackageManagerTabs
  code={`# migrate_and_serve.sh
#!/bin/sh

if [ $WEB_SERVICE ]; then
  TMP=/dev/shm NODE_ENV=production pnpm psy:js db:migrate --skip-sync && TMP=/dev/shm NODE_ENV=production pnpm psy:js db:seed && ./node_modules/.bin/pm2-runtime start ./server.ecosystem.config.cjs --env production && ./node_modules/.bin/pm2 save
elif [ $WORKER_SERVICE ]; then
  ./node_modules/.bin/pm2-runtime start ./worker.ecosystem.config.cjs --env production && ./node_modules/.bin/pm2 save
