...
if [ $WEB_SERVICE ]; then
  YARN_CACHE_FOLDER=/dev/shm TMP=/dev/shm NODE_ENV=production yarn psy db:migrate --skip-sync \
  && YARN_CACHE_FOLDER=/dev/shm TMP=/dev/shm NODE_ENV=production yarn psy db:seed \
