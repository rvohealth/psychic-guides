...
if [ $WEB_SERVICE ]; then
  TMP=/dev/shm NODE_ENV=production pnpm psy:js db:migrate --skip-sync && TMP=/dev/shm NODE_ENV=production pnpm psy:js db:seed && ./node_modules/.bin/pm2-runtime start ./api.ecosystem.config.cjs --env production
