...
RUN pnpm workspaces focus --production
...
RUN rm .npmrc
RUN echo "nodeLinker: node-modules" > .npmrc
...
USER node
RUN echo 'alias pnpm="corepack pnpm"' >> /home/node/.profile
...
if [ $WEB_SERVICE ]; then
  PNPM_CACHE_FOLDER=/dev/shm TMP=/dev/shm NODE_ENV=production pnpm psyjs db:migrate --skip-sync \
  && PNPM_CACHE_FOLDER=/dev/shm TMP=/dev/shm NODE_ENV=production pnpm psyjs db:seed \
