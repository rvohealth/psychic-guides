...
Under the hood, psychic provides typed test helpers to make controller and endpoint specs line up with generated OpenAPI paths:
...
Projects commonly wrap authenticated requests in a `session(user)` helper that returns an `OpenapiSpecRequest` with the right cookies or headers applied:

```ts
describe('an authed user attempts to hit an authed route', () => {
  it('returns 200', async () => {
    const user = await createUser()
    await session(user).get('/api/v1/users/me', 200)
  })
})
```

Negative ownership specs still need the authenticated role row. If a route is scoped to `currentHost`, create a host for the signed-in user before asserting that some other host's record is forbidden or hidden.
