...
ai_summary: Feature specs (end-to-end/e2e) test interactions outside backend using headless browser (Puppeteer). Written from backend context but drive frontend client. Uses Vitest. Configuration in spec/features/vite.config.ts. Launches Vite dev server for client. Programmatically starts/stops Psychic server.
---
