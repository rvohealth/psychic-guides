...
We also need to configure the jest-playwrite package, which we can do by providing a second file, `spec/feature/jest-playwrite.config.js`. This file is used to launch both your backend _and_ your frontend in conjunction. Once both instances are up, it will begin running your feature specs, which can drive a headless browser through your frontend, hitting your backend for complete end-to-end testing.
...
module.exports = {
  ...
  serverOptions: [
    ...
    {
      command:
        'FEATURE_SPEC_RUN=1 DEV_SERVER_PORT=7778 npx ts-node --transpile-only ./src/spec-server.ts',
      host: '127.0.0.1',
      launchTimeout:
        (process.env.LAUNCH_TIMEOUT_SECONDS &&
          parseInt(process.env.LAUNCH_TIMEOUT_SECONDS) * 1000) ||
        60000,
      debug: process.env.DEBUG === '1',
      port: 7778,
      usedPortAction: 'kill',
