...
By default, Psychic encrypts any cookies that are created through it's cookie api. The encryption algorithm and key for this can be set in `conf/app.ts`, like so:
...
Within Dream, an `Encrypted` decorator is provided, which will automatically encrypt any values that are bound to it's call. In order for it to work properly, you must provide an algorithm and key in the `conf/dream.ts` file, like so:
