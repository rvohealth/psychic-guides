...
STI child models support the same column types as the [model generator](/docs/models/generating). Child-specific columns are added to the parent table; STI children should not declare their own associations, `@SoftDelete()`, `@Sortable()`, or `@ReplicaSafe()` decorators.
