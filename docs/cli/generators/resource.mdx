...
Run `pnpm psy <command> --help` before running any generator — argument formats vary between commands and between Dream/Psychic versions, so don't infer syntax from these examples or prior experience.
...
3. Commit the generated code as its own commit before hand-editing it, so the hand edits are easy to review separately.
