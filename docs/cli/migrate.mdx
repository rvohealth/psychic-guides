...
sidebar_position: 6
ai_summary: >-
  Database migration commands include db:migrate (runs outstanding migrations),
  db:rollback (rolls back migrations with --steps option), db:create (creates
  database), db:drop (drops database), db:reset (drops, creates, migrates, and
  seeds), and db:seed (seeds database). All support --skip-sync option.
---
...
<PackageManagerTabs code={`NODE_ENV=test {{PM}} psy db:rollback`} />

Roll back multiple migrations at once:

<PackageManagerTabs code={`NODE_ENV=test {{PM}} psy db:rollback --steps 3`} />
...
<PackageManagerTabs code={`NODE_ENV=test {{PM}} psy db:integrity-check`} />
