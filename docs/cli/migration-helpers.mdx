...
All migration helpers are available from the `DreamMigrationHelpers` class:
...
The `renameTable` helper automatically renames both the table and its associated sequence (for tables with serial primary keys), ensuring consistency across your database schema.
...
export async function down(db: Kysely<any>): Promise<void> {
  await DreamMigrationHelpers.dropConstraint(db, 'constraint_name', {
    table: 'table_name',
  })
}
```
...
Safely drops a constraint from a table. This method provides a consistent interface for constraint removal across different constraint types.

```ts
export async function up(db: Kysely<any>): Promise<void> {
  // Remove a constraint that's no longer needed
  await DreamMigrationHelpers.dropConstraint(db, 'old_constraint_name', {
    table: 'table_name',
  })
}
...
export async function down(db: Kysely<any>): Promise<void> {
  ...
  await DreamMigrationHelpers.addDeferrableUniqueConstraint(db, 'old_constraint_name', {
    table: 'table_name',
    columns: ['col1', 'col2'],
  })
}
...
Creates GIN (Generalized Inverted Index) indexes, which are particularly useful for full-text search and similarity operations.
